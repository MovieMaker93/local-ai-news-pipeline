...
## When to use
Called by orchestrator AFTER editor writes `/tmp/lain/edition.json`. Generates illustrations for the lead story and one per non-empty section.
...
2. For each image target (lead + each non-empty section):
   ...
   g. Save image to `/tmp/lain/images/` with deterministic name.
      **Check the `image` field returned by `image_generate`** — it may be a local file path
      ...
      - If local path → `cp "<path>" /tmp/lain/images/lead_<date_iso>.jpg`
      - If remote URL → `curl -sL "<url>" -o /tmp/lain/images/lead_<date_iso>.jpg`
      ...
      **CRITICAL: never skip this download/copy step.** The image exists only in the cloud
      or cache until you save it to `/tmp/lain/images/`.
