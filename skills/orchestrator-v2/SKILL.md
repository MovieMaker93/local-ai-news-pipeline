...
Archive current edition (self-contained snapshot)    ← AFTER reset, BEFORE copy
  ↓
Copy new files → save edition.json → update headline history → copy podcast audio → git add → commit → push
```
...
9. **Step 12d — Copy podcast audio:** Podcast .ogg from `/tmp/v2/podcasts/` is copied to `$DEPLOY_DIR/podcasts/`. The `podcasts/` directory is created in the deploy dir. **CRITICAL:** This step was missing until 2026-07-09 — the podcast file was generated and the HTML referenced it, but the .ogg was never copied, so the player appeared on the page but produced no sound. The fix: added `mkdir -p "$DEPLOY_DIR/podcasts" && cp /tmp/v2/podcasts/*.ogg "$DEPLOY_DIR/podcasts/"` to the deploy copy block in `run_v2.sh`.
...
- **Podcast audio must be copied to deploy dir** — the HTML references `podcasts/<filename>.ogg` but the file only exists in `/tmp/v2/podcasts/` after generation. The deploy step MUST copy it to `$DEPLOY_DIR/podcasts/` or the player renders silently. Fixed in `run_v2.sh` on 2026-07-09 with `mkdir -p "$DEPLOY_DIR/podcasts" && cp /tmp/v2/podcasts/*.ogg "$DEPLOY_DIR/podcasts/"`.
...
**New wrapper path (since 2026-07-09):** The cron job's `script` field was changed from `v2/cron_wrapper.sh` (symlink, resolved outside scripts dir) to `lux-v2-cron.sh` (direct file in scripts dir, which `exec`s the real wrapper). This avoids the `realpath` resolution check that blocked symlinked scripts.
...
### 17. Podcast .ogg files not copied to deploy dir
**Discovered 2026-07-09:** The HTML references `podcasts/lead_<date>.ogg` but the deploy step only copied `index.html`, fonts, images, and `edition.json` — podcast audio was left behind in `/tmp/v2/podcasts/`. The player rendered silently.

**Symptoms:**
- Podcast pill player appears in the HTML
- Clicking play shows no error but no sound
- `ls ~/ai-news-deploy/podcasts/` shows only old files, not today's

**Fix applied:** Added to the deploy copy block in `run_v2.sh`:
```bash
mkdir -p "$DEPLOY_DIR/podcasts"
cp /tmp/v2/podcasts/*.ogg "$DEPLOY_DIR/podcasts/" 2>/dev/null || true
```

**Recovery (if a run already completed but podcast is missing):**
```bash
cp /tmp/v2/podcasts/lead_$(date +%Y-%m-%d).ogg ~/ai-news-deploy/podcasts/
cd ~/ai-news-deploy && git add podcasts/ && git commit -m "Add podcast for $(date +%Y-%m-%d)" && git push
```
...
### First Response — \"Why didn't today's release go up?\" (with podcast check)
...
# 4. Check podcast file was copied
ls -lh ~/ai-news-deploy/podcasts/lead_$(date +%Y-%m-%d).ogg 2>/dev/null || echo \"Podcast missing from deploy — run recovery\"

# 5. Check git
cd ~/ai-news-deploy && git log --oneline -1

# 6. Check the issue counter
