...
Stage 2 (LLM, DeepSeek V4 Flash, dedicated model):
  scout-v2-youtube skill → read raw JSON → extract newsworthy items
...
## Stage 2: scout-v2-youtube (LLM)

**Skill:** `scout-v2-youtube`
...
The `editor-v2` skill has a **per-section item threshold** — YouTube & Video stays as a full section with **≥2 items**. With 0-1 items, it collapses into Quick Hits.
